...
from timm.models.layers import ClassifierHead, SelectAdaptivePool2d
...
class ConvMlp(BaseModule):
    ...
    def __init__(
        ...
        self.fc1 = Conv2d(in_features, hidden_features, kernel_size=1, bias=True)
        self.norm = norm_layer(hidden_features) if norm_layer else nn.Identity()
        self.act = act_layer()
        self.fc2 = Conv2d(hidden_features, out_features, kernel_size=1, bias=True)
...
class ConvNeXtBlock(BaseModule):
    ...
    def __init__(self, dim, drop_path=0., ls_init_value=1e-6, conv_mlp=True, mlp_ratio=4, norm_layer=None):
        ...
        self.conv_dw = Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)  # depthwise conv
...
class ConvNeXtStage(BaseModule):
    ...
    def __init__(
        ...
        if in_chs != out_chs or stride > 1:
            self.downsample = Sequential(
                norm_layer(in_chs),
                Conv2d(in_chs, out_chs, kernel_size=stride, stride=stride if not no_downsample else 1),
            )
        else:
            self.downsample = nn.Identity()

        dp_rates = dp_rates or [0.] * depth
        self.blocks = Sequential(*[ConvNeXtBlock(
...
class ConvNeXt(BaseModule):
    ...
    def __init__(
        ...
        self.stem = Sequential(
            Conv2d(in_chans, dims[0], kernel_size=patch_size, stride=patch_size),
            norm_layer(dims[0])
        )
        ...
        if head_norm_first:
            ...
            if use_head:
                self.head = Sequential(OrderedDict([
                    ...
                    ('fc', Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity())
                ]))
